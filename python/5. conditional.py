...
for num in numbers:
    print(f"{num} is {determine_sign(num)}")
...
def check_eligibility(age, residency):
    ...
    senior_age = 80
    senior_residency = "Local"

  
    if age >= senior_age and residency == senior_residency:
        return True
    else:
        return False
...
ages = [55, 65, 70]
residencies = ["Local", "Foreign", "Local"]

for i in range(len(ages)):
    eligibility = check_eligibility(ages[i], residencies[i])
    if eligibility:
        print(f"At age {ages[i]}, residing locally, the person is eligible for a senior citizen discount.")
    else:
        print(f"At age {ages[i]}, residing locally, the person is not eligible for a senior citizen discount.")
...
def login(username, password):
    ...
    correct_username = "dipto420"
    correct_password = "121121"
